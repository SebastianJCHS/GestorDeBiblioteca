...
public class ArregloPrestamo {
    ...
    public PrestacionLibro[] Regresarprestamos() {
        PrestacionLibro[] listaPrestamos = new PrestacionLibro[indice];
        System.arraycopy(this.Prestamo, 0, listaPrestamos, 0, indice);
    ...
    public PrestacionLibro buscarPrestamo(String tituloLibro, int numeroEjemplar) {
        PrestacionLibro[] todoslosprestamos = Regresarprestamos();
        for (PrestacionLibro prestamo : todoslosprestamos) {
            if (prestamo.getLibro().getNombre().equalsIgnoreCase(tituloLibro)) {
            // Verifica si el número del ejemplar coincide
                Ejemplar ejemplar = prestamo.getEjemplar();
                if (ejemplar != null && ejemplar.getID_Ejemplar() == numeroEjemplar) {
                    return prestamo; // Devuelve el préstamo correcto
                }
            }
        }
